...
function styleButtons() {
    ...
    $('#new_mail').button();
    $('#new_mail').bind('click', function () { window.location.href = 'compose.aspx' });
    $('#contacts').button({
        icons: {
    ...
    $('#contacts').bind('click', function () { window.location.href = 'contacts.aspx' });
...
function loadTags() {
    ...
    $.getJSON('api/Tags', function (tags) {
        $('#tag_section').html('');
        $('#tag_section').append('<a href="#" onclick="AddNewTag()">Add New Tag</a><br/>');
...
function updateTag() {
    $.ajax({
        url: 'api/Tags/' + t_id + '?newname=' + $('#tag_name_box').val(),
        type: 'PUT',
        success: function (result) {
            loadTags();
            dialog.dialog('close');
...
function loadAllThreads() {
    ...
    $('.threadDelete').button({ icons: { primary: "ui-icon-trash" } });
    $('.threadReply').button({ icons: { primary: "ui-icon-pencil" } });
    $('.threadAddTag').button({ icons: { primary: "ui-icon-tag" } });
    $('.threadRemoveTag').button({ icons: { primary: "ui-icon-tag" } });
}

function AddTag(acc, id) {
    var tag = window.prompt("Enter tag to add:");
    if (tag == null || tag == "")
        return;
    window.location.href = 'index.aspx?add=' + tag + '&thread=' + id + '&acc=' + acc;
}

function RemoveTag(acc, id) {
    var tag = window.prompt("Enter tag to add:");
    if (tag == null || tag == "")
        return;
    window.location.href = 'index.aspx?remove=' + tag + '&thread=' + id + '&acc=' + acc;
}